...
function DocPanel({ step, isSecondDoc }) {
  ...
  const pickConfig = useCallback((item) => {
    ...
    updateStep(step.id, { config: item, fb: fbValid ? currentFb : null });
  }, [step.id, step.fb, updateStep]);

  const pickPath = useCallback((path) =>
    updateStep(step.id, { path, config: null, fb: null }), [step.id, updateStep]);
  ...
  return (
    <div className="panel-body">
      ...
      {/* Coherence checks — secondary second doc */}
      {(() => {
        ...
        if (!coherenceOptions.length) return null;
        return (
          <PSection title="Coherence checks">
