...
// Inline module picker — shown at end of flow
function InlineModulePicker({ types, onSelect, onCancel }) {
  const ref = useRef(null);
          ...
          return (
            <button key={type} className="inline-mod-picker-item" onClick={() => onSelect(type)}>
              ...
              <span className="inline-mod-picker-info">
                <span className="inline-mod-picker-name">{mod.name}</span>
                <span className="inline-mod-picker-desc">{mod.desc}</span>
              </span>
            </button>
          );
        })}
...
// Add module button (end of flow)
function AddModuleBtn({ onOpen }) {
  return (
    <button className="add-backup-btn" onClick={(e) => { e.stopPropagation(); onOpen(); }}>
      <svg viewBox="0 0 12 12" fill="none" width="11" height="11">
        <path d="M6 1v10M1 6h10" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
...
export default function Canvas() {
  ...
  // Compute available module types for inline add (excluding doc — handled by backup btn)
  const usedTypes = new Set(pipeline.map((s) => s.type));
  const allCandidates = mode === 'auth' ? ['auth'] : ['face', 'ext', 'int'];
  const inlineAddTypes = allCandidates.filter((t) => !usedTypes.has(t));
  ...
  return (
    <>
      <div ref={canvasRef} className="canvas" onDragOver={onDragOver} onDrop={onDrop} onClick={handleCanvasClick}>
        {pipeline.length === 0 ? (
          ...
          <div className="flow" style={{ transform: `scale(${zoom})`, transformOrigin: 'top center', transition: 'transform .15s' }}>
            ...
            {/* Inline add module — shown when there are still modules to add */}
            {inlineAddTypes.length > 0 && (
              <>
                {showModPicker ? (
                  <InlineModulePicker
                    types={inlineAddTypes}
                    onSelect={(type) => { setShowModPicker(false); addStep(type); }}
                    onCancel={() => setShowModPicker(false)}
                  />
                ) : (
                  <AddModuleBtn onOpen={() => setShowModPicker(true)} />
                )}
                <Connector />
              </>
            )}
